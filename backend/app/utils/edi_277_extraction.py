...
def validate_277_file(file_content: str) -> tuple[bool, str]:
    """
    Validate 277 EDI file structure (X12 277 claim status response).
    ...
    try:
        ...
        if not content.startswith("ISA"):
            return False, "Invalid 277 EDI: Must start with ISA segment"

        if "IEA" not in content:
            return False, "Invalid 277 EDI: Missing IEA segment"

        # 277 transaction set identifier (ST*277*...)
        if not re.search(r"ST\*277\*", content):
            return False, "Invalid 277 EDI: Not a 277 transaction set (expected ST*277*)"

        return True, "Valid 277 EDI file"
    except Exception as e:
        return False, f"277 validation error: {str(e)}"
...
if __name__ == "__main__":
    INPUT_EDI = "CHA075331-R20260120-682.edi"
    FIXED_EDI = "fixed.edi"
