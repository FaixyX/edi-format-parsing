...
import {
    useBillingFilesStore,
    type UploadResult,
} from "@/stores/billingFilesStore";
import {
...
export default function Format1BillingFilesPage() {
    const {
        selectedFiles,
        uploadResults,
        setSelectedFiles,
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const response = await myFetch("POST", "billing-files/upload", {
            ...
            if (data.successful > 0 && data.failed === 0) {
                toast.success(
                    `Successfully uploaded ${data.successful} file(s)`,
                    {
                        description: `${data.task_ids.length} task(s) created and queued for processing`,
                    }
                );
    ...
    const handleRetry = async (result: UploadResult, index: number) => {
    ...
    const retryAllFailed = async () => {
        ...
        const storeState = useBillingFilesStore.getState();
        ...
        if (filesToReupload.length > 0) {
            try {
                const files: File[] = [];
                const fileResultMap = new Map<string, UploadResult>();

                // Get fresh state to check for files
                const currentStoreState = useBillingFilesStore.getState();
                console.log(
                ...
                if (files.length === 0) {
                    ...
                    const freshState = useBillingFilesStore.getState();
    ...
    return (
        <div className="space-y-6">
            <Card className="p-0 m-0">
                <CardHeader>
                    <CardTitle>Upload EDI Billing Files</CardTitle>
                    <CardDescription>
                        Upload X12 EDI 835 billing files (.edi or .txt). Each
                        file will be processed as a separate task.
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                    ...
                    <Alert>
                        ...
                        <AlertDescription>
                            Each file must contain a valid NPI in the N1*PE
                            segment. The system will automatically match files
                            to agencies by NPI.
                        </AlertDescription>
                    </Alert>
